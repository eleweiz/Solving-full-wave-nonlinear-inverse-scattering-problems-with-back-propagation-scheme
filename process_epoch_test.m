function  [net_cpu,stats,prof] = process_epoch_test(opts, patchSize, epoch, subset, learningRate, imdb, net_cpu,Coef)
% -------------------------------------------------------------------------
    ...
    for s=1:opts.numSubBatches
        ...
        if training
            if (mod(t,20) == 1)%&&(rand>0.95)
                displayImg_test(labels,im,res,lowRes,opts,Coef);
            end
        else
